...
import org.easydarwin.easyplayer.activity.PlayRTMPActivity;
 ...
 * rtmp播放器Fragment
 * */
public class PlayRTMPFragment extends Fragment implements TextureView.SurfaceTextureListener, PhotoViewAttacher.OnMatrixChangedListener {
    protected static final String TAG = PlayRTMPFragment.class.getSimpleName();
    ...
    protected EasyRTMPPlayerClient mStreamRender;
    ...
    public static PlayRTMPFragment newInstance(String url, int type, ResultReceiver rr) {
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, url);
        args.putInt(ARG_PARAM2, type);
        args.putParcelable(ARG_PARAM3, rr);

        PlayRTMPFragment fragment = new PlayRTMPFragment();
        fragment.setArguments(args);

        return fragment;
    }

    public static PlayRTMPFragment newInstance(Context context, String url, ResultReceiver rr) {
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, url);

        boolean useUDP = SPUtil.getUDPMode(context);

        args.putInt(ARG_PARAM2, useUDP ? RTMPClient.TRANS_TYPE_UDP : RTMPClient.TRANS_TYPE_TCP);
        args.putParcelable(ARG_PARAM3, rr);
...
//                    }
                    ...
                    if (activity instanceof PlayRTMPActivity) {
                        int state = resultData.getInt("state");
                        String msg = resultData.getString("event-msg");
                        ((PlayRTMPActivity) activity).onEvent(PlayRTMPFragment.this, state, errorCode, msg);
                    }
                } else if (resultCode == EasyRTMPPlayerClient.RESULT_RECORD_BEGIN) {
                    if (activity instanceof PlayRTMPActivity)
                        ((PlayRTMPActivity)activity).onRecordState(1);
        ...
        GestureDetector.SimpleOnGestureListener sgl = new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onDoubleTap(MotionEvent e) {
                if (doubleTapListener != null)
                    doubleTapListener.onDoubleTab(PlayRTMPFragment.this);
            ...
            public boolean onSingleTapUp(MotionEvent e) {
                if (doubleTapListener != null)
                    doubleTapListener.onSingleTab(PlayRTMPFragment.this);
...
//         Toast.makeText(PlayRTMPActivity.this, "视频正在播放了", Toast.LENGTH_SHORT).show();
    ...
    protected void startRending(SurfaceTexture surface) {
        mStreamRender = new EasyRTMPPlayerClient(getContext(), KEY, new Surface(surface), mResultReceiver);
    ...
    private void onVideoSizeChange() {
        ...
        if (mRatioType == ASPECT_RATIO_CROPS_MATRIX) {
            ...
            mAttacher.setOnMatrixChangeListener(PlayRTMPFragment.this);
            mAttacher.update();
...
//        return true;
    ...
    public interface OnDoubleTapListener {
        void onDoubleTab(PlayRTMPFragment f);
        void onSingleTab(PlayRTMPFragment f);
    }
